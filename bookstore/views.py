   ...
   if request.method == 'POST':       
       ...
       if form.is_valid():                                
          ...
          #then use pre-defined Django function to login
               login(request, user)                
               return redirect('books:list') #& send the user to desired page
       else:                                               #in case of error
           error_message ='ooops.. something went wrong'   #print error message

   #prepare data to send from view to template
   context ={                                             
       'form': form,                                 #send the form dataerror_message
       'error_message': error_message                     #and the 
   }
...
def logout_view(request):                                  
   ...
   return redirect('sales:home')    #after logging out go to login form